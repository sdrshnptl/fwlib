...
int main(int argc, char *argv[]) {
    ...
    ODBST statinfo;
    ODBPRO odbpro;
    IODBPSD part_counter, tool_offset, feed_rate, spindle_speed, power_data;
    ODBALM alarm_data;
    ODBPOS position_data[3];  // Assuming we are reading position of 3 axes
    ODBSPLOAD spindle_load;
    ODBSVLOAD servo_load;
    char json_output[JSON_BUFFER_SIZE];
    char current_time[50], power_on_time[50], operating_time[50];
    ...
    // PART COUNTER: Retrieve part counter value (using parameter 6712)
    ...
    // FEED RATE: Retrieve feed rate (use appropriate parameter or method)
    ...
    if ((ret = cnc_rdposition(libh, 0, sizeof(position_data), position_data)) != EW_OK) {
        fprintf(stderr, "Failed to read position data! (%d)\n", ret);
        return 1;
    }

    // TOOL DATA: Retrieve tool offset (use parameter for tool offset)
    if ((ret = cnc_rdparam(libh, 5002, 0, 8, &tool_offset)) != EW_OK) {
        fprintf(stderr, "Failed to read tool data! (%d)\n", ret);
        return 1;
    }

    // SPINDLE LOAD METER & MOTOR SPEED: Retrieve spindle load
